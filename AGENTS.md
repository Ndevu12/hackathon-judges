...
- AI helpers: `ai/run_ai.py` (orchestrator) + `ai/providers.py` (pluggable backends) plus prompt assets in `ai/hackathon_context.md` and `ai/prompt_template.txt`.
...
- Put secrets in an untracked `.env` (copy `.env.example`): `HACKATHON_API_KEY` (scan) and, only for the `anthropic` AI provider, `ANTHROPIC_API_KEY`. Every script loads `.env` automatically. All scripts take `--config config.json`; settings resolve as CLI flag > config.json > built-in default.
...
- `python3 ai/run_ai.py --config config.json [--only-id <repo>]`: AI authenticity analysis with structured output → `work/ai_outputs/<id>.json` (reads `work/submissions.json`). Backend is `ai.provider`: `claude_code` (default) runs on the Claude Code CLI + Pro/Max **subscription** (needs the `claude` CLI + login, no API key); `anthropic` uses the Claude API (needs `pip install -r requirements.txt` + `ANTHROPIC_API_KEY`). `--provider`/`--model`/`--base-url`/`--api-key` override.
...
- Secrets (`HACKATHON_API_KEY`, `ANTHROPIC_API_KEY`) come from the environment or an untracked `.env`, never `config.json` or git. `config.json` holds only public settings (times, thresholds, paths, provider, model, base URLs).
...
- AI analysis is pluggable via `ai.provider`. Default `claude_code` shells out to the Claude Code CLI (`claude -p`) on the user's subscription — it deliberately scrubs `ANTHROPIC_API_KEY` from the child env so it can't silently bill the API. `anthropic` calls the Claude API directly. Provider, model, and `base_url` are configurable via the `ai` section.
