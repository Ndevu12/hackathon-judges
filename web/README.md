...
`work/` artifacts (metrics + `submissions.json`) and renders the submissions
table, red-flag columns, AI verdicts, team members, live URLs, and a per-repo
detail drawer.
...
from the repo-root `../config.json` on each request.

Point it at other data with env vars (see `.env.example`):

```bash
HJ_DATA_ROOT=/path/to/a/checkout npm run dev   # reads <root>/work
```
...
1. **Env** — `HJ_DATA_ROOT` (repo layout) or `HJ_WORK_DIR`.
2. **Live work dir** — from `../config.json` (`paths.work_dir`). Used during
   local `npm run dev`.
...
lib/            data.ts (source resolver), types, csv, verdict, format
